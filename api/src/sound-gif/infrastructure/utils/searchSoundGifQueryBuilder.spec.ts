...
describe('searchSoundGifQuery', () => {
  ...
  it.each([
    [
      undefined,
      undefined,
      { queryBuilder: 1, where: 0, andWhere: 0, orderBy: 0, getMany: 1 },
    ],
    [
      undefined,
      'tags',
      { queryBuilder: 1, where: 0, andWhere: 1, orderBy: 0, getMany: 1 },
    ],
    [
      { category: 'sport' },
      undefined,
      { queryBuilder: 1, where: 1, andWhere: 0, orderBy: 0, getMany: 1 },
    ],
    [
      { category: 'sport' },
      'zidane',
      { queryBuilder: 1, where: 1, andWhere: 1, orderBy: 0, getMany: 1 },
    ],
    [
      { category: 'sport' },
      'zidane',
    ...
    'should call the repo with filters %s, fulltext: %s',
    async (filters, fulltext, expectedResults) => {
      await searchSoundGifQuery(mockSoundGifRepository, filters, fulltext);
      expect(mockSoundGifRepository.createQueryBuilder).toHaveBeenCalledTimes(
        expectedResults.queryBuilder,
      );
